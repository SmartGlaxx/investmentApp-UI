...
const fetchCreatedUsers=async(url2)=>{
    ...
    usersCreated.filter(user =>{
         if(state.authenticated){
            ...
            if(user.email === state.loginDetail || user.username === state.loginDetail){
                ...
                dispatch({type: USERFULLDATA, payload: user})
                if(user){
                    localStorage.setItem('userFullData', JSON.stringify(user))   
                 }else{
  ...
  useEffect(()=>{
    ...
    return <AppContext.Provider value={{
        ...
        setLoginDetail, setUserDetail, setUserCreated, setUserVerified, setPackageData, setInvestmentDeposited,
        setRegistered
    }}>
    {children}
    </AppContext.Provider>
